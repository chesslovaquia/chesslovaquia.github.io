...
- **css_load:** Each theme defines its own `params.css_load` in its `config.yaml` (`themes/clvq/config.yaml`, `themes/clvq2/config.yaml`). Hugo's config merge uses the active theme's list as the default since the site-level config (`config/_default/config.yaml`) does not define `css_load`. Theme CSS lives in `themes/<theme>/assets/`; chessground CSS comes from the site-level module mount (`node_modules/chessground` -> `assets/chessground`). JS/TS params (`js_load`, `game_ts`, `sw_js`, etc.) are shared across themes and remain in `config/_default/config.yaml`.
