...
| lichess API | Phases 1–7 done | OAuth2 PKCE auth (`LichessAuth`); HTTP client with token injection + 429 handling (`LichessClient`); NDJSON streaming with reconnect (`LichessStream`); game flow — seek (incl. correspondence via `days`), challenge, resign, draw, takeback, `makeMove` (`LichessGame`); board integration — `LichessGameState` (idempotent `load()`), `EventOpponentMove`, `EventGameOver`, `GameClock.syncTimes()`, `ChessGame` online-mode wiring (`onMove`, `playerColor`, `doOpponentMove`); UI — unified play mode selector, challenge modal, opponent info panel, game actions bar (abort/resign/draw); game history — `GameHistory` (IndexedDB), `LichessHistory` (fetch from API); reconnect — active game ID persisted in `localStorage` key `lichess_game_id` (via `LichessUIBridge`), game page (`game.ts`) detects and recreates full lichess stack on reload, event stream runs on game page so `gameFinish` clears the key; playing games — `LichessPlaying` fetches `GET /api/account/playing`, home page panel shows ongoing games with Resume button (`Clvq.lichessResumeGame`, `Clvq.lichessFetchPlaying`) |
...
- Active lichess game ID is persisted in `localStorage` key `lichess_game_id`. `LichessUIBridge` owns read/write/delete of this key (set on `gameStart`, cleared on `gameFinish`, restored in constructor). `game.ts` and `Clvq.lichessResumeGame` use the same string constant locally — keep them in sync if the key ever changes.
- `game.ts` checks `localStorage` for `lichess_game_id` on init. If found and a user is logged in, it creates the full lichess stack and calls `lichessState.load()` before constructing `ChessGame` (so `playerColor` is known). If the user is not logged in, the key is cleared and the game falls back to local mode. `LichessGameState.load()` is idempotent — the second call from `ChessGame.init()` returns immediately.
- `LichessUIBridge` constructor now requires a `ClvqLocalStorage` second argument. Tests use the `makeBridge(auth)` helper in `LichessUIBridge_test.ts`. `Clvq.ts` passes `ls` (its own `ClvqLocalStorage`) to `new LichessUIBridge(this.auth, ls)`.
- `game_test.ts` must clear `localStorage` in `beforeEach`/`afterEach` — otherwise a stale `lichess_game_id` from another test in the same worker could trigger the lichess init path.
- `Clvq.lichessFetchPlaying()` is async — call it from `main.ts` with `.catch(...)` after constructing `window.Clvq`. Do not call it in the `Clvq` constructor (would cause HTTP fetch in tests that set `lichess_token`).
