...
- Responsive layouts: both `play/desktop.md` and `play/mobile.md` use `layout: game`; `screen.ts` redirects between `/play/desktop/` and `/play/mobile/` based on viewport width. The `clvq` theme provides `page/game.html` (desktop two-column layout); the `clvq2` theme provides a single responsive CSS grid layout that replaces both.
...
- **clvq2 theme** (in-progress, Phase 1 complete): `themes/clvq2/` — no w3.css, CSS custom properties (`variables.css`), single responsive CSS grid layout (`layout.css`), modal/dropdown via `.w3-show` hook in `components.css`. Switch by setting `theme = "clvq2"` in `hugo.toml`. Vendored fontawesome and board texture CSS are in top-level `assets/` for shared access.
...
- **clvq2 modal/menu system:** `w3ShowModal`/`w3HideModal`/`w3ToggleMenu` in `ts/clvq/utils.ts` toggle the `.w3-show` CSS class. The `clvq2` theme must define `.modal-container.w3-show { display: grid }` and `.game-dropdown.w3-show { display: flex }` in `components.css` to replace the w3.css display toggling. The pawn promotion modals are a special case: TypeScript sets `style.display = 'block'/'none'` directly — they must start with `style="display:none"` inline, not `class="modal-container"`.
- **clvq2 fontawesome path:** Vendored fontawesome CSS must live at `assets/fontawesome/css/` (not `assets/css/fontawesome/`) so that the `url("../webfonts/fa-solid-900.woff2")` reference in `solid.css` resolves correctly to `static/fontawesome/webfonts/fa-solid-900.woff2`.
